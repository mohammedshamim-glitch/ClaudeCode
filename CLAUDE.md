...
- **Fixed character roster — 5 characters, rotate 2 per episode**: There are exactly 5 named characters used across all Monkey Finance episodes. Pick 2 per episode. Never create new characters. Never reuse the same pair in consecutive episodes. Characters: Emma, Tom, Sarah, James, Priya. Full appearance descriptions and standing-pose prompts are in `references/CHARACTERS.md` in the scriptwriter skill folder.
- **Character reference file**: For every episode with named characters, create `00b-character-references.txt` in the episode Drive folder immediately after the script is approved. Include: one one-liner standing-pose Grok prompt per character used in that episode (copied from `CHARACTERS.md`), colour identity, scenes they appear in, and usage notes. File named `00b-` so it sorts near the top. Characters are 2D whiteboard animation style — NOT photorealistic. **Each character gets exactly one standing-pose prompt — no action poses, no 4-pose sheets.**
