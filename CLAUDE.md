...
- **TTS Pro model has ~11-minute audio output cap**: `gemini-2.5-pro-preview-tts` silently truncates output at ~655s (~11 min). A 13-minute script will be cut off without warning — the audio just ends mid-sentence. Pro must also use chunking. Flash is the default; Pro+chunking is an option if prosody quality matters, but costs ~2.5× more on input tokens.
