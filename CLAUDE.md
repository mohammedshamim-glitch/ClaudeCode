...
- **Pipeline stage order (FIXED — non-negotiable)**: Script → TTS → aeneas timings → **SEO (run immediately after timings, before images)** → images (Sham generates in Grok) → video assembly (`create_video_kb.py --subs`) → upload. NEVER ask Sham if he wants SEO run — just run it automatically after aeneas completes. SEO only needs `audio_timings_new.csv` for chapter timestamps; do NOT wait for video assembly.
...
- **Character appearance rule**: Never describe character clothing, hair, or physical appearance in image prompts. Only describe action, props, and facial expression. Reference images handle visual identity.
- **Character facial expressions — mandatory**: Every Sophie/Chris scene must specify a contextually appropriate facial expression. Match the emotion of the narration: anxious/worried for bad scenarios, confident/relieved for wins, surprised/concerned for reveals, thoughtful/pleased for realisations. A scene with a character and no expression note is a failed prompt.
