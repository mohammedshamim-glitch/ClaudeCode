...
- **TTS chunk persistence — mandatory**: This is an ephemeral container. After all chunks are generated for an episode, `run_tts.py` must upload each chunk WAV to a `tts_chunks/` subfolder inside the episode's Drive folder. At the start of any TTS run, check Drive for existing chunks first — download any that exist before generating. This prevents losing chunks to session restarts and avoids wasting quota re-generating audio that was already correct. Never delete chunk files from Drive until the episode is fully assembled and uploaded.
...
- **Pipeline stage order (FIXED — non-negotiable)**: Stage 2 Script → Stage 3 Image prompts (`04-image-prompts.txt` + `00b-character-references.txt` if named chars) → Stage 4 TTS (`narration.mp3`) → Stage 5a aeneas timings + SEO + community post draft (`06-seo-metadata.txt`, `07-thumbnail-prompt.txt`) → Stage 5b video assembly (`create_video_kb.py --subs`) → Stage 7 upload → Stage 8 Short → Stage 9 wrap up (immediately after Short, no gate). Community post is drafted at Stage 5a inside `06-seo-metadata.txt` — post manually in Studio on publish day. **Approval gate after every stage except 8→9 — always wait for Sham's sign-off before proceeding.** Never skip a stage. Never run stages out of order.
...
- **Image prompt format — ALWAYS invoke monkey-finance-image-prompts skill**: Never write image prompts without invoking the skill first. The correct format is: `X.Y 2D colourful whiteboard animation style. Clean white background. All critical elements within the central 60% of the frame — minimum 20% clear margin on all edges. 16:9 widescreen aspect ratio, landscape composition. [scene description] Bold colourful hand-drawn illustration.` — all in ONE paragraph, blank line between scenes, NO "Scene N:" headers, NO section labels. Scene 1.1 and final scene = green suit monkey. ~20% scenes monkey-free (stat/data dominant). Stats MUST be written explicitly on canvas. End every prompt with "Bold colourful hand-drawn illustration."
- **Image sort — leading number must come first**: `create_video_kb.py` sort_key always tries the leading `NNN_` prefix before any trailing number. If an image filename has a timestamp suffix (e.g. `_202606181752.jpg`), the trailing regex would extract that huge number and sort the image to the wrong position. Leading prefix is the authoritative sort key. The sequencing validation also hard-stops if the first sorted image is not `001_`.
...
- **Flash TTS pace variance — pace-lock is mandatory**: Without intervention, Flash chunks range 145–195 wpm (a 50 wpm swing), producing audibly inconsistent audio. `run_tts.py` includes a `pace_lock()` pass that measures each chunk's wpm and uses `ffmpeg atempo` (pitch-preserving tempo stretch) to slow fast chunks down to the median wpm ceiling. Slow chunks are left at their natural pace — never sped up. Cap is ±35% (0.75–1.0 range; speedup is always skipped). This is automatic — do not remove it.
