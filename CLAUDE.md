...
- **ffmpeg/ffprobe broken on this host**: libcaca0 security update `0.99.beta20-4ubuntu0.1` returns 404 from Ubuntu mirrors. Fix: `apt-get install -y libcaca0=0.99.beta20-4build2` — pins to the stable noble/main version. After that, `apt-get install -y libespeak-dev` and `pip install aeneas` both succeed. Run this fix at the start of any new session before using ffmpeg, ffprobe, or aeneas.
