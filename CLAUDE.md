...
- **Pipeline stage order (updated)**: After TTS, run Whisper timings → then SEO. Video assembly (create_video_kb.py) runs separately after images are generated. Do NOT block SEO on video assembly — SEO only needs `audio_timings_new.csv` for chapter timestamps.
- **ffmpeg/ffprobe broken on this host**: libcaca0 is missing from Ubuntu mirrors causing ffmpeg and ffprobe to crash with shared library error. All scripts use capability checks (`subprocess.run(["ffmpeg", "-version"]).returncode == 0`) and fall back to Python alternatives: WAV duration via `wave` module; WAV→MP3 via `pymp3`; audio resampling via `scipy.signal.resample_poly`. Never use `shutil.which()` alone to check if ffmpeg works — the binary exists but crashes.
- **Whisper timing fallback**: If aeneas is unavailable (libespeak-dev blocked by libcaca0 dependency), use Whisper for forced alignment instead. Install via `pip install openai-whisper`. Uses the `base` model (~139MB).
