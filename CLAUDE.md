...
- **Fixed character roster — 5 characters, 0–3 per episode**: There are exactly 5 named characters used across all Monkey Finance episodes: Emma, Tom, Sarah, James, Priya. Each episode uses 0, 1, 2, or 3 of them — never more than 3. Some videos have no named characters at all (monkey only). Never create new characters outside the five. Avoid reusing the exact same combination in consecutive episodes. Full appearance descriptions and standing-pose prompts are in `references/CHARACTERS.md` in the scriptwriter skill folder.
